...
class SequentialWorkflow(WorkflowTemplate):
    ...
    def define_workflow(self) -> None:
        ...
        step1 = self.add_step(
            "Generate topic about {{instruction}}",
...
# Use structured output in your workflow
builder = WorkflowBuilder(instruction="Write an article about AI")

builder.generative_step(
    id="generate_structured_article",
    prompt="Write an article about {{instruction}} with sections and tags",
    operator=Operator.GENERATION,
    function=structured_output(schema),
    outputs=[Write.new("structured_article")],
)

flow = [Edge(source="generate_structured_article", target="_end")]
builder.flow(flow)
```

## Model Selection

Dria allows you to select specific models for different tasks in your workflows.

```python
from dria.models import ModelSelector

# Create a workflow with specific model selection
builder = WorkflowBuilder(instruction="Analyze complex scientific text")
...
builder.generative_step(
    id="summarize",
    prompt="Summarize this analysis in simple terms: {{analysis}}",
    operator=Operator.GENERATION,
    model=ModelSelector.claude_3_haiku(),  # Specify a faster model
    outputs=[Write.new("summary")],
)

flow = [
    Edge(source="complex_analysis", target="summarize"),
