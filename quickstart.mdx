...
- Each step builds on the output of the previous step using the `{{$last_result}}` template variable
- The special `$last_result` template variable automatically inserts the output from the previous step
